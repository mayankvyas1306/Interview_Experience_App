    ...
    return (
        <div className="container py-5">
            <div className="d-flex gap-3 align-items-center mb-4 flex-wrap">
                ...
                <Link href="/admin/users" className="btn btn-outline-secondary rounded-3 btn-sm">Users</Link>
                <Link href="/admin/posts" className="btn btn-outline-secondary rounded-3 btn-sm">Posts</Link>
            </div>
            ...
            {loading ? (
                ...
                <div className="glass rounded-4 p-4 text-center">
                    ...
                    <button onClick={fetchReports} className="btn btn-accent">Retry</button>
                </div>
            ) : reports.length === 0 ? (
                            ...
                            <div className="d-flex justify-content-between flex-wrap gap-3">
                                <div>
                                    <div className="fw-bold mb-1">
                                        {r.postId?.companyName || "Deleted Post"} — {r.postId?.role || ""}
                                    </div>
                                    <div className="text-muted2 small mb-1">
                                        Reported by: {r.reporterId?.fullName || "Unknown"} ({r.reporterId?.email})
                                    </div>
                                    <div className="d-flex gap-2 flex-wrap">
                                        <span className="badge bg-warning text-dark">{r.reason}</span>
                                    ...
                                    {r.details && (
                                        <p className="text-muted2 small mt-2 mb-0">
                                            <i className="bi bi-chat-left-text me-1"></i>{r.details}
                                        </p>
                                    )}
                                </div>
