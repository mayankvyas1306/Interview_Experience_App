...
export default function AIPrepPage() {
    ...
    const fetchPrepGuide = async () => {
        ...
        try {
            ...
            setPrepGuide(res.data as AIPrepGuide);
        } catch (err: unknown) {
            setPrepError(
                err instanceof Error ? err.message : "Failed to generate guide"
            );
        } finally {
    ...
    const fetchComparison = async () => {
        ...
        try {
            ...
            setComparison(res.data as AIComparison);
        } catch (err: unknown) {
            setCompareError(
                err instanceof Error ? err.message : "Failed to compare"
            );
        } finally {
    ...
    const fetchPracticeQuestions = async () => {
        ...
        try {
            ...
            setPracticeSet(res.data as AIPracticeSet);
        } catch (err: unknown) {
            setPracticeError(
    ...
    const toggleHint = (index: number) => {
        setRevealedHints((prev) => {
            ...
            // ✅ Fixed: removed the bare expression that caused line 104 warning
            return next;
        });
    };
    ...
    return (
        <div className="container py-5">
            ...
            >
                <h1 className="fw-bold display-5 mb-2">✨ AI Interview Prep</h1>
            ...
            <div className="d-flex justify-content-center flex-wrap gap-2 mb-5">
                {[
                    { id: "prep" as Tab, label: "📋 Prep Guide" },
                    { id: "compare" as Tab, label: "⚖️ Compare" },
                        ...
                        {prepGuide && (
                            ...
                            >
                                ...
                                <div className="glass rounded-4 p-4">
                                    ...
                                    <div className="d-flex flex-column gap-2">
                                        {prepGuide.guide.key_topics.map((item, i) => {
                                            const color =
                                                IMPORTANCE_COLOR[item.importance] || "#888";
                                            return (
                                                <div
                                                    key={i}
                                                    className="glass rounded-3 p-3 d-flex gap-3 align-items-start"
                                                >
                                                    <span
                                        ...
                                        <div className="glass rounded-4 p-4 h-100">
                                            ...
                                            <ul className="mb-0 ps-3">
                                                {prepGuide.guide.tips.map((tip, i) => (
                                                    <li key={i} className="text-light small mb-2">
                                                        {tip}
                                                    </li>
                                                ))}
                                            </ul>
                                        ...
                                        <div className="glass rounded-4 p-4 h-100">
                                            ...
                                            <ul className="mb-0 ps-3">
                                                {prepGuide.guide.red_flags.map((flag, i) => (
                                                    <li key={i} className="text-warning small mb-2">
                                                        {flag}
                                                    </li>
                                                ))}
                                            </ul>
                                ...
                                <p className="text-muted2 small text-center">
                                    ...
                                    {new Date(prepGuide.generatedAt).toLocaleDateString()}
                                </p>
                            </motion.div>
                        )}
                    </motion.div>
                                ...
                                <div className="glass rounded-4 p-4">
                                    ...
                                    <div className="table-responsive">
                                        <table className="table table-dark table-bordered align-middle">
                                            ...
                                            <tbody>
                                                {comparison.comparison.process_comparison.map(
                                                    (row, i) => (
                                                        <tr key={i}>
                                                            <td className="fw-semibold text-muted2">
                                                                {row.aspect}
                                                            </td>
                                                            <td className="small">
                                                                {row[comparison.company1]}
                                                            </td>
                                                            <td className="small">
                                                                {row[comparison.company2]}
                                                            </td>
                                                        </tr>
                                                    )
                                                )}
                            ...
                            >
                                {practiceSet.basedOnRealData && (
                                    <div className="glass rounded-3 p-3 mb-4 d-flex align-items-center gap-2">
                                        ...
                                        <span className="text-muted2 small">
                                            Generated using real interview questions from the
                                            community
                                        </span>
                                    </div>
                                )}
                                ...
                                <div className="d-flex flex-column gap-3">
                                    {practiceSet.questions.map((q, i) => {
                                        const color = DIFFICULTY_COLOR[q.difficulty] || "#888";
                                        return (
                                            <motion.div
                                                key={i}
                                ...
                                <p className="text-muted2 small text-center mt-4">
                                    ✨ Generated by Gemini AI •{" "}
                                    {new Date(practiceSet.generatedAt).toLocaleDateString()}
                                </p>
                            </motion.div>
                        )}
                    </motion.div>
